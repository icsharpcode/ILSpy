...
namespace Ricciolo.StylesExplorer.MarkupReflection
{
	class TypeDeclaration
	{
		...
		public TypeDeclaration(XmlBamlReader reader, ITypeResolver resolver, string name, string namespaceName, short assemblyId)
		{
			...
			if (!IsExtension)
				IsExtension = name.EndsWith("Extension");
		}
		...
		public string FullyQualifiedName => EnclosingTypeName == null ? string.Format("{0}.{1}", Namespace, Name) : string.Format("{0}.{1}+{2}", Namespace, EnclosingTypeName, Name);

		public string AssemblyQualifiedName => string.Format("{0}, {1}", FullyQualifiedName, Assembly);

		public override bool Equals(object obj)
		{
			var td = obj as TypeDeclaration;
	...
	class ResolverTypeDeclaration : TypeDeclaration
	{
		...
		{
			var bracket = assemblyQualifiedName.LastIndexOf(']');
			var commaSeparator = bracket > -1 ? assemblyQualifiedName.IndexOf(", ", bracket) : assemblyQualifiedName.IndexOf(", ");
			assembly = "";
			if (commaSeparator >= 0) {
			...
			var namespaceSeparator = assemblyQualifiedName.LastIndexOf('.');
			@namespace = "";
		...
		public override string Assembly { get; }

		public override bool Equals(object obj)
		{
			var td = obj as ResolverTypeDeclaration;
			if (td != null)
